...
static int map_one_page(struct proc *p, struct mmap_area *ma, uint64 va)
{
  ...
  if(ma->f){
    ...
    if(readi(ma->f->ip, 0, (uint64)mem, off, toread) < 0){
      iunlock(ma->f->ip);
      end_op();
      kfree(mem);
      return -1;
...
do_munmap(uint64 addr)
{
  ...
  uvmunmap(p->pagetable, start, ma->length/PGSIZE, 1);
...
// clone parent's mmap areas into child during fork().
// copy already-populated pages, leave lazy pages unmapped.
void
fork_mmaps(struct proc *parent, struct proc *child)
{
  ...
  for(int i=0;i<MMAP_MAX;i++){
    if(mmap_areas[i].p == parent){
      ...
      int j = -1;
      for(int k=0;k<MMAP_MAX;k++){
        if(mmap_areas[k].p == 0){ j = k; break; }
      }
      if(j < 0){
        // out of slots: best effort, skip
        continue;
      }
      mmap_areas[j] = mmap_areas[i];
      mmap_areas[j].p = child;
      if(mmap_areas[i].f)
        mmap_areas[j].f = filedup(mmap_areas[i].f);
      // copy mapped pages that already exist in parent
      release(&mmap_lock);
      for(uint64 a = mmap_areas[i].addr; a < mmap_areas[i].addr + mmap_areas[i].length; a += PGSIZE){
        ...
        if(ppte && (*ppte & PTE_V)){
          ...
          // ensure user accessibility preserved
